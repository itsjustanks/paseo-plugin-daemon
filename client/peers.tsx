...
export function Peers({ hostLabel }: { hostLabel: string }) {
  ...
  return <>
    <Notice icon="Laptop">Local ports are created on {hostLabel}. Choose the daemon running on your computer to open remote apps on your localhost.</Notice>
    ...
    {!!query.data?.peers.length && <Section title="Paired hosts">
      ...
      {serviceQuery.isError && <Notice icon="WifiOff" tone="warning" action={<Button label="Retry" onPress={() => { void serviceQuery.refetch(); }} />}>{errorText(serviceQuery.error)} You can also use Fallback on the remote host.</Notice>}
      {serviceQuery.data && !serviceQuery.data.services.length && <Card><Text style={t.text.body}>No services are listening yet. Start a dev server on the paired host; it will appear here.</Text></Card>}
      {serviceQuery.data?.services.map((service) => {
        ...
        return <Card key={service.port}>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            ...
            {active && <Button label="Disconnect" onPress={() => mutation.mutate(() => disconnect({ id: active.id }))} />}
          </View>
        </Card>;
      })}
    </Section>}
    {!!query.data?.forwards.length && <Section title="Local ports">
      {query.data.forwards.map((f) => <Card key={f.id}><Text style={t.text.body}>localhost:{f.localPort} → remote port {f.remotePort}</Text><Button label="Disconnect" onPress={() => mutation.mutate(() => disconnect({ id: f.id }))} /></Card>)}
    </Section>}
    ...
    {(pairing || !query.data?.peers.length) && <Section title="One-time pairing">
      <Card>
        ...
        <Text style={t.text.body}>Create a code here to allow your computer to discover and connect to this host's services. Share it only with a device you trust.</Text>
      ...
      <Card>
        ...
        <Text style={t.text.body}>Switch to your computer using Paseo's host picker, then paste the code here. Pairings are saved across restarts.</Text>

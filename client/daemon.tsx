...
type Tab = "local" | "relay" | "health" | "guide";
...
function Choice<Id extends string>({ items, selected, onChange, label }: { items: { id: Id; label: string; icon: string; description: string }[]; selected: Id; onChange(id: Id): void; label: string }) {
  const t = useTokens();
  return <View accessibilityRole="tablist" accessibilityLabel={label} style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
    {items.map((item) => <Pressable key={item.id} accessibilityRole="tab" accessibilityLabel={item.label} accessibilityState={{ selected: selected === item.id }} onPress={() => onChange(item.id)}
      style={({ pressed }) => ({ flexGrow: 1, flexBasis: t.compact ? "44%" : 180, minHeight: 68, padding: 12, gap: 5, borderRadius: 10, borderWidth: 1, borderColor: selected === item.id ? t.color.accent : t.color.border, backgroundColor: selected === item.id ? alpha(t.color.accent, 0.09) : pressed ? t.color.surface2 : t.color.surface1 })}>
...
function DaemonBody(props: DaemonProps) {
  ...
  const [tab, setTab] = useState<Tab>("local"), [route, setRoute] = useState<Route>("private");
  ...
  const links = useQuery({ queryKey: ["daemon-link", "status"], queryFn: () => status({}), refetchInterval: 3000, retry: 1 });
  const peers = useQuery({ queryKey: ["daemon-link", "peers"], queryFn: () => peerStatus({}), refetchInterval: 3000, retry: 1 });
  const local = useQuery({ queryKey: ["daemon-link", "services"], queryFn: () => monitor.snapshot({ query: "", sort: "name", limit: 25 }), refetchInterval: 5000, enabled: tab !== "health", retry: 1 });
  ...
  const navigate = (next: Tab, nextRoute: Route = "private") => { setTab(next); setRoute(nextRoute); };
  ...
  return <View style={{ flex: 1, backgroundColor: t.color.surface0 }}>
    <View style={{ padding: t.space.lg, gap: t.space.md, width: "100%", maxWidth: t.maxWidth, alignSelf: "center" }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
        <View style={{ gap: 3, flexShrink: 1 }}><Text style={t.text.title}>Daemon Link</Text><Text style={t.text.label}>Selected host: {props.host.label}</Text></View>
    ...
    {tab === "health" ? <MonitorSurface {...props} /> : <ScrollView key={tab} contentContainerStyle={{ padding: t.space.lg, paddingTop: 4, gap: t.space.lg, width: "100%", maxWidth: t.maxWidth, alignSelf: "center" }}>
      ...
      {tab === "local" && <>
        ...
        {local.isPending ? <Text style={t.text.body}>Checking Paseo projects and their dev servers…</Text> : <ProjectApps apps={apps.filter((app) => `${app.project?.name} ${app.classification.label} ${app.ports.join(" ")}`.toLowerCase().includes(search.toLowerCase()))} onConnect={() => navigate("relay")} searching={!!search} />}
      ...
      {tab === "relay" && <>
        ...
        {route === "private" && <Peers hostLabel={props.host.label} onBrowserLink={() => setRoute("browser")} onGuide={() => navigate("guide")} />}
        {route === "browser" && <>
      ...
      {tab === "guide" && <>
        ...
        <Grid min={270}>
          ...
          <Step number="2" title="Install and pair both hosts"><Text style={t.text.body}>Install Daemon Link on the app's host and on your laptop or desktop. On the app's host, open Dev Relay → Private localhost → Create pairing code. Switch Paseo's host picker to your computer and paste the code there.</Text><Button label="Open pairing" onPress={() => navigate("relay")} /></Step>
          <Step number="3" title="Open the app from your computer"><Text style={t.text.body}>Keep your computer selected. Choose the paired host and press Open localhost beside its app. Use the displayed URL. If the same port is busy locally, Daemon Link chooses a free one.</Text><Text style={t.text.caption}>Close forward disconnects access; it does not stop the app. On phones, choose Temporary browser link instead.</Text></Step>
        </Grid>
...
function ProjectApps({ apps, onConnect, searching }: { apps: Process[]; onConnect(): void; searching: boolean }) {
  ...
  return <Grid min={330}>{groups.map((id) => {
    ...
    return <Card key={id}><View style={{ flexDirection: "row", justifyContent: "space-between", gap: 10 }}><Text style={[t.text.heading, { flexShrink: 1 }]}>{project.name}</Text><StatusPill tone="ok" label="Running" /></View><Text style={t.text.caption}>{project.path}</Text>
      ...
      <Button label="Access from another device" onPress={onConnect} /></Card>;
  })}</Grid>;
}
